...
const CATEGORIES = ['Electronics', 'Fashion', 'Home & Living', 'Beauty', 'Sports', 'Food & Groceries', 'Gaming', 'Health', 'Books'];
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.surfaceSoft }} edges={['top']}>
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 }}>
          ...
          <Text style={{ fontFamily: 'OpenSans_400Regular', fontSize: 14, color: colors.inkMuted, textAlign: 'center', marginBottom: 36 }}>
            Your product has been submitted for review and will be live shortly.
          </Text>
          <View style={{ width: '100%', gap: 12 }}>
            <Button title="Add Another Product" onPress={() => setSaved(false)} />
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 24 }}>
          {CATEGORIES.map(cat => (
            <Pressable
              ...
              style={{ paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, backgroundColor: selectedCategory === cat ? colors.ink : colors.surfaceSoft, borderWidth: 1.5, borderColor: selectedCategory === cat ? colors.ink : colors.surfaceMuted }}
            >
              <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 13, color: selectedCategory === cat ? colors.surface : colors.inkSoft }}>{cat}</Text>
            </Pressable>
          ))}
        </View>

        <Button title="Save Product" onPress={() => setSaved(true)} />
